...
public class Player
{
     ...
     *
     * @param addRingSize		number to add to players maxEnergy
     * @param addArchiveSize	number to add to players playerNumber
     * @param addResearchSize	number to add to players numberToResearch
     */
    public void upgrade(int addRingSize, int addArchiveSize, int addResearchSize)
    {
        if (addRingSize != 0)
            toolbar.upgrade(addRingSize, 0, 0);
        if (addArchiveSize != 0)
            toolbar.upgrade(0, addArchiveSize, 0);
        if (addResearchSize != 0)
            toolbar.upgrade(0, 0, addResearchSize);
    }
    ...
    {
        Iterator<Marble> iter = energyRing.iterator();
        while (iter.hasNext())
        {
            Marble m = iter.next();
    ...
    {
        int val = 0;
        Iterator<BonusVictoryPoint> iter = victoryPoints.iterator();
        while (iter.hasNext())
